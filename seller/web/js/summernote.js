...
(function (global, factory) {
    ...
    var dialog = renderer.create('<div class="modal modal-upload" aria-hidden="false" tabindex="-1"/>', function ($node, options) {
        if (options.fade) {
            $node.addClass('fade');
        }
        $node.html([
            ...
            (options.title
                    ...
                    '      <button type="button" class="close close_modal_upload" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
                    '      <h4 class="modal-title">Tải hình ảnh</h4>' +
                    '    </div>' : ''),
